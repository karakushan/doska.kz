...
class WPFB_Mailchimp_API_V3_Client {
	...
	*/
	private function request( $method, $resource, array $data = array() ) {
		...
		if ( empty( $this->api_key ) ) {
			throw new WPFB_Mailchimp_API_Exception( 'Missing API key', 001 );
		}
		...
		$args   = array(
			...
			'timeout'   => 20,
			'sslverify' => apply_filters( 'wpfb_mailchimp_use_sslverify', true ),
		);
		...
		* @param array $args
		*/
		$args = apply_filters( 'wpfb_mailchimp_http_request_args', $args, $url );
	...
	*/
	private function get_headers() {
		...
		$headers = array(
			'Authorization' => sprintf( 'Basic %s', base64_encode( 'wpfb_mailchimp:' . $this->api_key ) ),
			'User-Agent'    => sprintf( 'WordPress/%s; %s', $wp_version, home_url() ),
		);
	...
	* @return mixed
	*
	* @throws WPFB_Mailchimp_API_Connection_Exception|WPFB_Mailchimp_API_Resource_Not_Found_Exception|WPFB_Mailchimp_API_Exception
	*/
	private function parse_response( $response ) {
		if ( $response instanceof WP_Error ) {
			throw new WPFB_Mailchimp_API_Connection_Exception( $response->get_error_message(), (int) $response->get_error_code(), $this->last_request );
		}
		...
		if ( $code >= 400 ) {
			...
			if ( is_object( $data ) && isset( $data->type ) && $data->type === 'akamai_error_message' ) {
				throw new WPFB_Mailchimp_API_Connection_Exception( $message, $code, $this->last_request, $this->last_response, $data );
			}

			if ( $code === 404 ) {
				throw new WPFB_Mailchimp_API_Resource_Not_Found_Exception( $message, $code, $this->last_request, $this->last_response, $data );
			}

			// mailchimp returned an error..
			throw new WPFB_Mailchimp_API_Exception( $message, $code, $this->last_request, $this->last_response, $data );
		}

		// throw exception if unable to decode response
		if ( $data === null ) {
			throw new WPFB_Mailchimp_API_Exception( $message, $code, $this->last_request, $this->last_response );
		}
