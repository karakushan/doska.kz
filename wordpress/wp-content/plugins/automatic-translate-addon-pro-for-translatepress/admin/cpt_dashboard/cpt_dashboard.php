 ...
 * $dashboard=Tpap_Dashboard::instance();
 * }
 * 
 * Store options
 * if(class_exists('Tpap_Dashboard')){
 ...
 * if(class_exists('Tpap_Dashboard')){
 *  Tpap_Dashboard::get_translation_data(
 *      'prefix', // Required
 *      array(
...
if(!class_exists('Tpap_Dashboard')){
    class Tpap_Dashboard{
        ...
        public function __construct(){
            add_action('wp_ajax_tpap_hide_review_notice', array($this, 'tpap_hide_review_notice'));
        }
        ...
        public static function ctp_enqueue_assets(){
            if(function_exists('wp_style_is') && !wp_style_is('tpap-review-style', 'enqueued')){
                $plugin_url = plugin_dir_url(__FILE__);
                wp_enqueue_style('tpap-review-style', esc_url($plugin_url.'assets/css/cpt-dashboard.css'), array(), '1.0.0', 'all');
                wp_enqueue_script('tpap-review-script', esc_url($plugin_url.'assets/js/cpt-dashboard.js'), array('jquery'), '1.0.0', true);
            }
        }
        ...
        public static function review_notice($prefix, $plugin_name, $url){
            if(self::tpap_hide_review_notice_status($prefix)){
                return;
            }
            ...
            add_action('admin_notices', function() use ($message, $prefix, $url){
                ...
                $html .= '<div class="cpt-review-notice-content"><p>'.wp_kses_post($message).'</p><div class="tpap-review-notice-dismiss" data-prefix="'.esc_attr($prefix).'" data-nonce="'.esc_attr(wp_create_nonce('tpap_hide_review_notice')).'"><a href="'.esc_url($url).'" target="_blank" class="button button-primary">Rate Now! ★★★★★</a><button class="button cpt-already-reviewed">'.esc_html__('Already Reviewed', 'cp-notice').'</button><button class="button cpt-not-interested">'.esc_html__('Not Interested', 'cp-notice').'</button></div></div></div>';
                
                echo wp_kses_post($html);
            });

            add_action('tpap_display_admin_notices', function() use ($message, $prefix, $url){
        ...
        public static function tpap_hide_review_notice_status($prefix){
        ...
        public function tpap_hide_review_notice(){
            ...
            if(wp_verify_nonce($_POST['nonce'], 'tpap_hide_review_notice')){
