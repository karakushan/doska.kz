    ...
    static languageSupportedStatus = async (sourceLanguage, targetLanguage, targetLanguageLabel, sourceLanguageLabel) => {
        ...
        if (!window.hasOwnProperty('chrome') || !navigator.userAgent.includes('Chrome') || navigator.userAgent.includes('Edg')) {
            const message = jQuery(`<span style="color: #ff4646; display: inline-block;">
                ...
                <div style="text-align: right;">
                    <button onclick="location.reload()" class="tpap-error-reload-btn">Reload Page</button>
                </div>
            </span>`);
            return message;
        }
        ...
        if (!('translation' in self && 'createTranslator' in self.translation) && !('ai' in self && 'translator' in self.ai ) && !("Translator" in self && "create" in self.Translator)) {
            const message = jQuery(`<span style="color: #ff4646; display: inline-block;">
                ...
                <p>If you continue to experience issues after following the above steps, please <a href="https://my.example.net/account/support-tickets/" target="_blank" rel="noopener">open a support ticket</a> with our team. We are here to help you resolve any problems and ensure a smooth translation experience.</p>
                <div style="text-align: right;">
                    <button onclick="location.reload()" class="tpap-error-reload-btn">Reload Page</button>
                </div>
                </span>`);
...
var tpapChromeAiInit=async ()=>{
    ...
    const completeTranslation = () => {
        ...
        setTimeout(() => {
            ...
            jQuery(".chrome-ai-translator-modal .tpap-stats").fadeIn("slow");
        }, 3000);
    }
        ...
        const TranslatorObject = await ChromeAiTranslator.Object(
            {
                ...
                btnText: (translatedLanguageName !== undefined) ? `Translate To ${translatedLanguageName} (Beta)` : `Translate`,
