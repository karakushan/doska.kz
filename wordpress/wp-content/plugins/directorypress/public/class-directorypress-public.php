...
class DirectoryPress_Public_Handler {
	...
	public function enqueue_global_vars() {
		...
		echo 'var directorypress_js_instance = ' . json_encode(
				array(
						'ajaxurl' => $ajaxurl,
						'nonce' => wp_create_nonce('directorypress-ajax-nonce'),
						'in_favourites_msg' => esc_html__('Bookmarked', 'DIRECTORYPRESS'),
						'not_in_favourites_msg' => esc_html__('Bookmark', 'DIRECTORYPRESS'),
						'ajax_load' => true,
						'is_rtl' => is_rtl(),
						...
						'send_button_text' => esc_html__('Send message', 'DIRECTORYPRESS'),
						'send_button_sending' => esc_html__('Sending...', 'DIRECTORYPRESS'),
						...
						'max_attchment_size_unit' => esc_html__('Kilobytes', 'DIRECTORYPRESS'),
						'max_attchment_size_error' =>  esc_html__('skipped, File size should not exceed', 'DIRECTORYPRESS'),
				)
		) . ';';
		// move to maps addon
	if (directorypress_has_map()){	
		...
		echo 'var directorypress_maps_instance = ' . json_encode(
				array(
						...
						'directorypress_map_info_window_button_readmore' => esc_html__('Read more »', 'DIRECTORYPRESS'),
						'directorypress_map_info_window_button_summary' => esc_html__('« Summary', 'DIRECTORYPRESS'),
						'draw_area_button' => '',
						'edit_area_button' => '',
						'apply_area_button' => '',
						'reload_map_button' => '',
						'enable_my_location_button' => (int)$DIRECTORYPRESS_ADIMN_SETTINGS['directorypress_address_geocode'],
						'my_location_button' => esc_html__('My Location', 'DIRECTORYPRESS'),
						'my_location_button_error' => esc_html__('GeoLocation service does not work on your device!', 'DIRECTORYPRESS'),
