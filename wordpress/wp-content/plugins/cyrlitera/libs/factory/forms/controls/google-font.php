<?php
	...
	class Wbcr_FactoryForms480_GoogleFontControl extends Wbcr_FactoryForms480_FontControl {
		...
		const APIKEY = 'YOUR_GOOGLE_API_KEY_REMOVED';
		...
		protected function getGoogleFonts()
		{
			...
			if( is_wp_error($response) ) {
				...
				$this->error = __('Unable to retrieve the list of Google Fonts.', 'wbcr_factory_forms_480');
			...
			if( !isset($response['body']) ) {
				...
				$this->error = __('Invalide response from the Google Fonts API.', 'wbcr_factory_forms_480');
			...
			if( empty($body->items) ) {
				...
				$this->error = __('Unexpected error. The list of Google Fonts are empty.', 'wbcr_factory_forms_480');
