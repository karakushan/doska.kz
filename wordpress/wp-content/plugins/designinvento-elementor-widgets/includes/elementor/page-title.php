...
class Pacz_Page_Title extends Widget_Base {
	...
	public function get_title() {
		return __( 'Page Title', 'designinvento-elementor-widgets' );
	}
	...
	protected function register_content_page_title_controls() {
		...
		$this->add_control(
			...
			[
				...
				'raw'             => sprintf( __( '<b>Note:</b> Archive page title will be visible on frontend.', 'designinvento-elementor-widgets' ) ),
		...
		$this->add_control(
			...
			[
				'label'        => __( 'Enable Full Width Container', 'designinvento-elementor-widgets' ),
				'type'         => Controls_Manager::SWITCHER,
				'1'     => __( 'Yes', 'designinvento-elementor-widgets' ),
				'0'    => __( 'No', 'designinvento-elementor-widgets' ),
				'description' => __( 'Section should be full width as well to make it work.', 'designinvento-elementor-widgets' ),
				'default'      => 0,
			]
		);
		$this->add_control(
			...
			[
				...
				'options' => [
					'title_breadcrumb' => __( 'Title + Breadcrumbs (title at top)', 'designinvento-elementor-widgets' ),
					'breadcrumb_title' => __( 'Breadcrumbs (Breadcrumbs at top)', 'designinvento-elementor-widgets' ),
					'title_only' => __( 'Title only', 'designinvento-elementor-widgets' ),
				],
		...
		$this->add_control(
			'heading_tag',
			[
				'label'   => __( 'HTML Tag', 'designinvento-elementor-widgets' ),
				'type'    => Controls_Manager::SELECT,
				'options' => [
					'h1' => __( 'H1', 'designinvento-elementor-widgets' ),
					'h2' => __( 'H2', 'designinvento-elementor-widgets' ),
					'h3' => __( 'H3', 'designinvento-elementor-widgets' ),
					'h4' => __( 'H4', 'designinvento-elementor-widgets' ),
					'h5' => __( 'H5', 'designinvento-elementor-widgets' ),
					'h6' => __( 'H6', 'designinvento-elementor-widgets' ),
				],
				'default' => 'h2',
			]
		);
		...
		$this->add_responsive_control(
			'title_align',
			[
				'label'              => __( ' Title Alignment', 'designinvento-elementor-widgets' ),
				'type'               => Controls_Manager::CHOOSE,
				'options'            => [
					'left'    => [
						'title' => __( 'Left', 'designinvento-elementor-widgets' ),
						'icon'  => 'eicon-text-align-left',
					],
					'center'  => [
						'title' => __( 'Center', 'designinvento-elementor-widgets' ),
						'icon'  => 'eicon-text-align-center',
					],
					'right'   => [
						'title' => __( 'Right', 'designinvento-elementor-widgets' ),
						'icon'  => 'eicon-text-align-right',
					],
					'justify' => [
						'title' => __( 'Justified', 'designinvento-elementor-widgets' ),
		...
		$this->add_responsive_control(
			'breadcrumbs_align',
			[
				'label'              => __( 'Breadcrumbs Alignment', 'designinvento-elementor-widgets' ),
				'type'               => Controls_Manager::CHOOSE,
				'options'            => [
					'left'    => [
						'title' => __( 'Left', 'designinvento-elementor-widgets' ),
	...
	protected function register_page_title_style_controls() {
		$this->start_controls_section(
			'container_styling',
			[
				'label' => __( 'Container', 'designinvento-elementor-widgets' ),
		...
		$this->add_group_control(
			...
			[
				...
				'label' => esc_html__( 'Background', 'designinvento-elementor-widgets' ),
		...
		$this->start_controls_section(
			...
			$this->add_control(
				'blend_mode',
				[
					'label'     => __( 'Blend Mode', 'designinvento-elementor-widgets' ),
					'type'      => Controls_Manager::SELECT,
					'options'   => [
						''            => __( 'Normal', 'designinvento-elementor-widgets' ),
						'multiply'    => 'Multiply',
						'screen'      => 'Screen',
						'overlay'     => 'Overlay',
						'darken'      => 'Darken',
		...
		$this->start_controls_section(
			...
			[
				'label' => __( 'Breadcrumbs', 'designinvento-elementor-widgets' ),
				'tab'   => Controls_Manager::TAB_STYLE,
			]
		);
	...
	public function pacz_page_title() {
		...
		if(is_home() && get_option('page_for_posts')){
			...
			$title = esc_html__( 'Blog', 'designinvento-elementor-widgets' );
		}
		...
		if(is_singular('post')) {
			...
			if (isset($pacz_settings['page-title-blog']) && $pacz_settings['page-title-blog'] == 1 ) {
				$title = esc_html__( 'Latest News', 'designinvento-elementor-widgets' );
			}
		}
		if ( is_search() ) {
			$title = esc_html__( 'Search', 'designinvento-elementor-widgets' );
		}
		if ( is_archive() ) {
			if ( is_category() ) {
				$title = sprintf( esc_html__( ' %s', 'designinvento-elementor-widgets' ), single_cat_title( '', false ) );
			}
			elseif ( is_tag() ) {
				$title = sprintf( esc_html__( ' %s', 'designinvento-elementor-widgets' ), single_tag_title( '', false ) );
			}
			elseif ( is_day() ) {
				$title = sprintf( esc_html__( ' %s', 'designinvento-elementor-widgets' ), get_the_time( 'F jS, Y' ) );
			}
			elseif ( is_month() ) {
				$title = sprintf( esc_html__( '%s', 'designinvento-elementor-widgets' ), get_the_time( 'F, Y' ) );
			}
			elseif ( is_year() ) {
				$title = sprintf( esc_html__( ' %s', 'designinvento-elementor-widgets' ), get_the_time( 'Y' ) );
			}
			elseif ( is_author() ) {
				$title = esc_html__( 'Author Profile', 'designinvento-elementor-widgets' );
			}
			elseif ( is_tax() ) {
				$term = get_term_by( 'slug', get_query_var( 'term' ), get_query_var( 'taxonomy' ) );
				$title = sprintf( esc_html__( ' %s', 'designinvento-elementor-widgets' ), $term->name );
			}
		}
		if ( is_404() ) {
			$title = esc_html__('404 Not Found', 'designinvento-elementor-widgets');
	...
	public function pacz_theme_breadcrumbs() {
        ...
        if ( !is_front_page() ) {
	        echo '<a href="';
				echo esc_url(home_url('/'));
					echo '">'.esc_html__('Home', 'designinvento-elementor-widgets');
	        echo "</a>";
        }
		...
		echo esc_html($delimiter) . esc_html__( 'Search results for &ldquo;', 'designinvento-elementor-widgets' ) . get_search_query() . '&rdquo;';

		} elseif ( is_tag() ) {

				echo esc_html($delimiter) . esc_html__( 'Tag &ldquo;', 'designinvento-elementor-widgets' ) . single_tag_title('', false) . '&rdquo;';

		} elseif ( is_author() ) {

			$userdata = get_userdata(get_the_author_meta('ID'));
			echo  esc_html($delimiter) . esc_html__( 'Author:', 'designinvento-elementor-widgets' ) . ' ' . $userdata->display_name;
		...
		if ( get_query_var( 'paged' ) )
			echo esc_html($delimiter) .' (' . esc_html__( 'Page', 'designinvento-elementor-widgets' ) . ' ' . get_query_var( 'paged' ) . ')';
        	...
        	$item = array();
				...
				elseif ( bbp_is_single_topic() ) {
					...
					if ( bbp_is_topic_split() )
						$item[] = esc_html__( 'Split', 'designinvento-elementor-widgets' );

					elseif ( bbp_is_topic_merge() )
						$item[] = esc_html__( 'Merge', 'designinvento-elementor-widgets' );

					elseif ( bbp_is_topic_edit() )
						$item[] = esc_html__( 'Edit', 'designinvento-elementor-widgets' );
				}
