...
function difp_include_require_files() {
	...
	if( is_admin() ) {
		$difp_files['settings'] 	= DIFP_PLUGIN_DIR. 'admin/class-directorypress-frontend-messages-admin.php';
	}			
...
//function difp_translation()
	...
	//load_plugin_textdomain('directorypress-frontend-messages', false, dirname( plugin_basename( __FILE__ ) ) . '/languages' );
	//}
...
function difp_enqueue_scripts()
	...
	wp_localize_script( 'difp-attachment-script', 'difp_attachment_script', 
			array( 
				'remove' => esc_js(__('Remove', 'directorypress-frontend-messages')),
				'maximum' => esc_js( difp_get_option('attachment_no', 4 ) ),
				'max_text' => esc_js( sprintf( __( 'Maximum %s allowed', 'directorypress-frontend-messages' ), sprintf(_n('%s file', '%s files', difp_get_option('attachment_no', 4 ), 'directorypress-frontend-messages'), number_format_i18n(difp_get_option('attachment_no', 4 )) ) ) )
	...
	wp_localize_script( 'difp-shortcode-newmessage', 'difp_shortcode_newmessage', 
			array( 
				...
				'refresh_text'	=> __( 'Refresh this page and try again. ', 'directorypress-frontend-messages' ),
			) 
		);
...
function difp_error($wp_error){
	...
	foreach($errors as $error){
		$html .= '<strong>' . __('Error', 'directorypress-frontend-messages') . ': </strong>'.esc_html($error).'<br />';
	}
	$html .= '</div>';
	return $html;
}
...
function difp_format_date( $date, $d = '' )
    {
		...
		if ( '0000-00-00 00:00:00' === $post->post_date ) {
			$h_time = __( 'Unpublished', 'directorypress-frontend-messages' );
		} else {
			$m_time = $post->post_date;
			$time = strtotime( $post->post_date_gmt );
			
			if ( ( abs( $t_diff = time() - $time ) ) < DAY_IN_SECONDS ) {
				$h_time = sprintf( __( '%s ago', 'directorypress-frontend-messages' ), human_time_diff( $time ) );
			} else {
...
function difp_send_message( $message = null, $override = array() )
{
	...
	if( ! empty($message['difp_parent_id'] ) ) {
		...
		$message['message_title'] = __('RE:', 'directorypress-frontend-messages'). ' ' . get_the_title( $message['post_parent'] );
		if( 'threaded' != difp_get_message_view() )
...
if(strlen($searchq)>0)
{
	...
	if (! empty( $user_query->results ))
	{
		...
		echo "<li>".__("No matches found", 'directorypress-frontend-messages')."</li>";
	echo "</ul>";
}
}
die();
...
function difp_notification() 
		{
			...
			$sm = sprintf(_n('%s unread message', '%s unread messages', $unread_count, 'directorypress-frontend-messages'), number_format_i18n($unread_count) );

				$show = '';
				
				$unread_ann_count = difp_get_user_announcement_count( 'unread' );
				$sa = sprintf(_n('%s unread announcement', '%s unread announcements', $unread_ann_count, 'directorypress-frontend-messages'), number_format_i18n($unread_ann_count) );
	
			if ( $unread_count || $unread_ann_count ) {
				$show = __("You have", 'directorypress-frontend-messages');
	
			if ( $unread_count )
				$show .= "<a href='".difp_query_url('messagebox')."'> $sm</a>";
	
			if ( $unread_count && $unread_ann_count )
				$show .= ' ' .__('and', 'directorypress-frontend-messages');
...
function difp_info_output(){
	...
	*/
	...
	if( difp_success()->get_error_messages() ) {
		...
		$html .= '<a href="#" class="close" data-bs-dismiss="alert" aria-label="'.esc_attr__("close", "directorypress-frontend-messages").'">&times;</a>';
	...
	if( difp_errors()->get_error_messages() ) {
		...
		foreach( difp_errors()->get_error_messages() as $e){
			$html .= '<strong>' . __('Error', 'directorypress-frontend-messages') . ': </strong>'.esc_html($e).'<br />';
		}
		$html .= '</div>';
	}
...
function difp_locate_template( $template_names, $load = false, $require_once = true ) {
	...
	$locations[30] = DIFP_PLUGIN_DIR . 'pro/public/';
	$locations[40] = DIFP_PLUGIN_DIR . 'public/';
	...
	switch( $action ) {
		...
		case 'newmessage' :
			if ( ! difp_current_user_can( 'send_new_message') ){
				difp_errors()->add( 'permission', __("You do not have permission to send new message!", 'directorypress-frontend-messages') );
				break;
			}
			...
			if( count( difp_errors()->get_error_messages()) == 0 ){
				if( $message_id = difp_send_message() ) {
					...
					if( 'publish' == $message->post_status ) {
						difp_success()->add( 'publish', __("Message successfully sent.", 'directorypress-frontend-messages') );
					} else {
						difp_success()->add( 'pending', __("Message successfully sent and waiting for admin moderation.", 'directorypress-frontend-messages') );
					}
				} else {
					difp_errors()->add( 'undefined', __("Something wrong. Please try again.", 'directorypress-frontend-messages') );
				}
			}
		...
		case 'reply' :
			...
			if ( ! difp_current_user_can( 'send_reply', $parent_id ) ){
				difp_errors()->add( 'permission', __("You do not have permission to send reply to this message!", 'directorypress-frontend-messages') );
				break;
			}
		...
		case 'bulk_action' :
			...
			if ( ! difp_verify_nonce( $token, 'bulk_action') ) {
				difp_errors()->add( 'token', __("Invalid Token. Please try again!", 'directorypress-frontend-messages') );
				break;
			}
		...
		case 'settings' :
			...
			if( count( difp_errors()->get_error_messages()) == 0 ){
				difp_success()->add( 'saved', __("Settings successfully saved.", 'directorypress-frontend-messages') );
			}
