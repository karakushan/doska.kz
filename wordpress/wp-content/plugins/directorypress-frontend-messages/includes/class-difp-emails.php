...
class Difp_Emails
  {
	...
	function send_email( $postid, $post ){
		...
		if( $participants && is_array( $participants ) )
		{
			...
			$subject =  get_bloginfo("name").': '.__('New Message', 'directorypress-frontend-messages');
			$message = __('You have received a new message in', 'directorypress-frontend-messages'). "\r\n";
			$message .= get_bloginfo("name")."\r\n";
			$message .= sprintf(__("From: %s", 'directorypress-frontend-messages'), difp_get_userdata( $post->post_author, 'display_name', 'id') ). "\r\n";
			$message .= sprintf(__("Subject: %s", 'directorypress-frontend-messages'),  $post->post_title ). "\r\n";
			$message .= __('Please Click the following link to view full Message.', 'directorypress-frontend-messages')."\r\n";
			$message .= directorypress_dashboardUrl(array('directory_action' => 'messages'))."\r\n";
	...
	function notify_users( $postid, $post ) {
		...
		$subject =  get_bloginfo("name").': '.__('New Announcement', 'directorypress-frontend-messages');
		$message = __('A new Announcement is Published in ', 'directorypress-frontend-messages')."\r\n";
		$message .= get_bloginfo("name")."\r\n";
		$message .= sprintf(__("Title: %s", 'directorypress-frontend-messages'), $post->post_title ). "\r\n";
		$message .= __('Please Click the following link to view full Announcement.', 'directorypress-frontend-messages'). "\r\n";
