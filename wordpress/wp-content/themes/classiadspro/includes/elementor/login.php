...
class Pacz_Elementor_Login extends Widget_Base {
	...
	public function scripts() {
			// Enqueue original user-menu styles from plugin
			wp_register_style( 'hfb-user-menu', PCPT_ASSETS_URL .'css/user-menu.css');
			wp_enqueue_style('hfb-user-menu');
			
			// Enqueue extended styles with message badges from theme
			wp_register_style( 'hfb-user-menu-extended', get_template_directory_uri() . '/includes/elementor/css/user-menu-extended.css', array('hfb-user-menu'));
			wp_enqueue_style('hfb-user-menu-extended');
	}
						...
						echo '<button class="dropbtn">';
							if(isset($settings['user_profile_image']) && $settings['user_profile_image'] == 'profile_photo'){
								echo '<div class="user-menu-icon">';
									...
									// Add message count badge on profile photo
									if (function_exists('difp_get_new_message_number')) {
										$unread_count = difp_get_new_message_number();
										if ($unread_count > 0) {
											echo '<span class="user-message-badge">' . esc_html($unread_count) . '</span>';
										}
									}
								echo '</div>';
