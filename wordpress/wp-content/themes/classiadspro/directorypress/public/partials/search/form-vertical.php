<?php 
	...
	echo '<form action="'. esc_url($search_url) .'" class="search-form-style1 directorypress-search-layout-'. esc_attr($search_form->form_layout) .' directorypress-content-wrap directorypress-search-form" data-id="'. esc_attr($search_form->form_id) .'" id="directorypress-search-form-'. esc_attr($search_form->form_id) .'">';
		...
		echo '<div class="directorypress-search-holder clearfix">';
			// Обёртка с заголовком Filters
			echo '<div class="search-filters-wrapper">';
				echo '<div class="search-filters-header" onclick="toggleSearchFilters(this)">';
					echo '<span>'. esc_html__('Filters', 'DIRECTORYPRESS') .'</span>';
					echo '<i class="fas fa-chevron-down"></i>';
				echo '</div>';
				echo '<div class="search-container clearfix" id="search-container-'. esc_attr($search_form->form_id) .'" style="margin-left:-'. esc_attr($search_form->args['gap_in_fields']) .'px; margin-right:-'. esc_attr($search_form->args['gap_in_fields']) .'px;">';
				...
				// Добавляем кастомный враппер для дополнительных фильтров
				echo '<div class="custom-filters-header"><span>Filters</span><i class="fas fa-chevron-down"></i></div>';
				echo '<div class="custom-filters-wrapper">';
				...
				echo '</div>'; // закрываем search-container
			echo '</div>'; // закрываем search-filters-wrapper
