 ...
 * - Сохраняет прогресс в progress.txt — при обрыве продолжит с последней строки
 * 
...
$Max            = 10;    // Максимум строк для обработки (для тестов; 0 = без ограничений)
// =====================================================

// ===================== ФАЙЛ ПРОГРЕССА =====================
$progressFile = __DIR__ . '/progress.txt';

// Читаем номер последней успешно обработанной строки
$startFromLine = 0;
if (file_exists($progressFile)) {
    $saved = trim(file_get_contents($progressFile));
    if (is_numeric($saved) && (int)$saved > 0) {
        $startFromLine = (int)$saved;
        echo "=== Возобновление с строки " . ($startFromLine + 1) . " ===</br></br>";
    }
}
// ==========================================================
...
$inputFile  = "sku_output_output_output_output_sahibinden.com_result.csv";
$outputFile = "id_sku_many_image_output_output_output_output_sahibinden.com_result.csv";
...
// Если возобновляем — открываем выходной файл на ДОЗАПИСЬ, иначе на запись
if ($startFromLine > 0 && file_exists($outputFile)) {
    $outputHandle = fopen($outputFile, 'a');
    echo "=== Режим дозаписи в: $outputFile ===</br></br>";
} else {
...
if ($header !== false) {
    // Пишем заголовок только при первом запуске (не при возобновлении)
    if ($startFromLine === 0) {
        fputcsv($outputHandle, $header);
        echo "Заголовок записан: " . implode(', ', array_slice($header, 0, 5)) . "...</br></br>";
    } else {
        echo "Заголовок пропущен (возобновление).</br></br>";
    }
}
...
while (($row = fgetcsv($inputHandle)) !== false) {
    ...
    if ($lineNumber <= $startFromLine) {
        echo "Строка $lineNumber: пропущена (уже обработана).</br>";
        continue;
    }

    // --- Ограничение по количеству строк ($Max) ---
    if ($Max > 0 && ($lineNumber - $startFromLine) > $Max) {
        echo "</br>Достигнут лимит $Max строк.</br>";
        break;
    }
    ...
    // --- Сохраняем прогресс после каждой успешно записанной строки ---
    file_put_contents($progressFile, $lineNumber);
    echo "  [ПРОГРЕСС] Сохранено: строка $lineNumber</br>";
