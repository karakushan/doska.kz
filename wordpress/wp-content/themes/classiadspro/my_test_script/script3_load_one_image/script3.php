 ...
 * - Каждый 7-й столбец начиная с 7 (индексы 7, 14, 21, ...) содержит URL изображения
 * - Скачивает изображение по URL
 * - Конвертирует изображение в WebP с сжатием
 * - Загружает в медиабиблиотеку WordPress через встроенные функции
 * - Записывает ID вложения вместо URL в новый CSV
...
// ===================== НАСТРОЙКИ =====================
$WEBP_QUALITY = 80;  // Качество сжатия WebP (0-100)
$TEMP_DIR     = __DIR__ . '/temp_images';  // Временная директория для скачанных и WebP файлов
$Max = 2;
// =====================================================
...
$outputFile = "output_output_sahibinden.com_result.csv";
 ...
 * @return string|false    Путь к скачанному файлу или false
 */
 ...
 * @return string|false      Путь к WebP файлу или false
 */
function convertToWebp(string $inputPath, string $tempDir, int $quality = 80): string|false
{
    ...
    // Загружаем изображение по формату
    switch ($mimeType) {
        case 'image/jpeg':
 ...
 * @return int|false       ID вложения или false
 */
function uploadToWordpress(string $filePath): int|false
{
    ...
    // Подготавливаем файл для загрузки через wp_handle_sideload
    $fileArray = [
    ...
    // Отключаем проверку tmp — файл не из $_FILES
    $overrides = [
        'test_form' => false,
        'test_type' => true,
    ];

    // Загружаем файл в wp-content/uploads
    ...
    // Генерируем метаданные (миниатюры и т.д.)
...
while (($row = fgetcsv($inputHandle)) !== false) {
    ...
    foreach ($row as $colIndex => &$value) {
        // Каждый 7-й столбец начиная с 7 (пропускаем столбец 0 — это Title)
        if ($colIndex > 0 && $colIndex % 7 === 0) {
            $imageUrl = trim($value);
            ...
            // 1. Определяем — URL или локальный путь
            ...
            // Удаляем скачанный оригинал (если был скачан)
            if (isset($localPath) && $localPath !== $imageUrl && file_exists($localPath)) {
                @unlink($localPath);
            }
            ...
            // 4. Заменяем URL на ID
            $value = $attachmentId;
            $uploaded++;
            echo "  [OK] URL заменён на ID: $attachmentId</br>";
        }
    }
    unset($value);
